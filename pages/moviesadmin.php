...
$limit = 10; // 10 movies per page as requested

// Xác định trang hiện tại
$current_page = isset($_GET['index']) && is_numeric($_GET['index']) && $_GET['index'] > 0 ? (int)$_GET['index'] : 1;
...
// Khởi tạo mảng cho danh sách phim theo trang
$listPhimtheoPage = [];
if ($total_records > 0) {
    for ($i = $offset; $i < min($offset + $limit, $total_records); $i++) {
        $listPhimtheoPage[] = $listphim[$i];
    }
}
...
// Hiển thị chi tiết phim nếu có MAPM
if (isset($_GET['MAPM'])) {
